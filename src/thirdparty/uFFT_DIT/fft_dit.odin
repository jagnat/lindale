package ufft_dit   // Decimation in time radix-2
...
fft_split :: proc ( x : [^]complex64, X : [^]complex64, N : u32, phi : complex64 ) {
	for n : u32 = 0; n < N / 2; n += 1 {
		X[ 0 / 2 + n ] = x[ 2 * n + 0 ] + x[ 2 * n + 1 ] * cmplx.exp_complex64( complex64( complex( 0.0, -2 * f32( math.PI ) ) ) * phi )
		X[ N / 2 + n ] = x[ 2 * n + 0 ] - x[ 2 * n + 1 ] * cmplx.exp_complex64( complex64( complex( 0.0, -2 * f32( math.PI ) ) ) * phi )
	}
}
